...
The world data for $^3$He and $^3$H described in Section ~\ref{sec:world_data} will be fitted with the sum of Gaussians parametrization described in Section ~\ref{sec:sog} in this section. This section will explain the choices made for each of the SOG fits including the number of Gaussians used to fit the world data and it will also describe the choices made involving the Q$_i$ fit parameters. The placement and spacing of the radii at which the Gaussians are place, R$_i$, will also be discussed as well as the method used to try to optimize the fits by adjusting the R$_i$ spacing while attempting to minimize $\chi^2$. 
