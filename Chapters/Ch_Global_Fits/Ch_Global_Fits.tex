...
Now examine the first set of fits from ~\ref{tab:3he_fits} visually. Figure ~\ref{fig:3he_fch_no_cut} shows the resulting charge form factors of each of the 1352 fits of the $^3$He world data without any $\chi^2_{max}$ cut plotted together. Plotted along with this analysis' fits in red is a blue line representing the average result from \cite{Article:Amroun} in the range that analysis considered its fits to be valid. The first feature to notice is that many of the fits are behaving in a nonphysical manner. We observe numerous dip only minima where the first sharp minima is expected. Numerous fits also have first or second minima located in the already well understood region of 20-30 fm$^{-2}$ where there are not expected to be minima. We want to eliminate these kinds of fits as we know them to be nonphysical from theory and prior measurements. 

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=1.1\linewidth]{Fch_3He_n12_1352.png}
\end{center}
\caption[\bf{Charge Form Factors from 1352 $^3$He Fits}]{
{\bf{Charge Form Factors from 1352 $^3$He Fits.}} These charge form factors (red lines) were derived from 1352 fits of the $^3$He world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. No $\chi^2_{max}$ cut was applied resulting in many nonphysical fits. The blue line is the fit from \cite{Article:Amroun}.}
\label{fig:3he_fch_no_cut}
\end{figure}

To eliminate these fits we can impose a cut, $\chi^2_{max}$, which removes fits with higher $\chi^2$ as described in Section ~\ref{ssec:ngaus}. Removing higher $\chi^2$ fits will generally remove the worse fits first hopefully eliminating the least physical looking results. A cut of $\chi^2_{max}$ = 500 on these fits results in the plot in Figure ~\ref{fig:3he_fch_cut}. By lowing $\chi^2_{max}$ until the last of the dip only minima were removed we have eliminated the nonphysical fits leaving only the desired `good' fits. Now it is clear that our results for the $^3$He charge form factor are in excellent agreement with \cite{Article:Amroun}. No changes were expected in the charge form factor due to the addition of more high Q$^2$ data as the charge form factor is dominated by the low Q$^2$ data which was already robust. If the new high Q$^2$ data is going to change our understanding of a form factor it is far more likely to be the magnetic form factor.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=1.1\linewidth]{Fch_3He_n12_852.png}
\end{center}
\caption[\bf{Charge Form Factors from 1352 $^3$He Fits $\chi^2_{max}$ = 500}]{
{\bf{Charge Form Factors from 1352 $^3$He Fits $\chi^2_{max}$ = 500.}} These charge form factors (red lines) were derived from 1352 fits of the $^3$He world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. A $\chi^2_{max}$ = 500 cut was applied removing the nonphysical fits leaving 852 `good' fits. The blue line is the fit from \cite{Article:Amroun}.}
\label{fig:3he_fch_cut}
\end{figure}

Figure ~\ref{fig:3he_fm_no_cut} shows the resulting magnetic form factors of each of the 1352 fits of the $^3$He world data without any $\chi^2_{max}$ cut plotted together. As with the charge form factor, with no cuts applied, there are many nonphysical fits. Most of these are identified by the location of their first minima being unreasonable, or by the minima not being sharp. It is expected that the magnetic form factor fits will be less clean than the charge form factor fits due to the relative lack of high Q$^2$ data. This is also why the $\chi^2_{max}$ cut was applied to the charge form factor instead of the magnetic. These fits can be greatly cleaned up once the $\chi^2_{max}$ cut is applied.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=1.1\linewidth]{Fm_3He_n12_1352.png}
\end{center}
\caption[\bf{Magnetic Form Factors from 1352 $^3$He Fits}]{
{\bf{Magnetic Form Factors from 1352 $^3$He Fits.}} These magnetic form factors (red lines) were derived from 1352 fits of the $^3$He world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. No $\chi^2_{max}$ cut was applied resulting in many nonphysical fits. The blue line is the fit from \cite{Article:Amroun}.}
...
\end{center}
\caption[\bf{Magnetic Form Factors from 1352 $^3$He Fits $\chi^2_{max}$ = 500}]{
{\bf{Magnetic Form Factors from 1352 $^3$He Fits $\chi^2_{max}$ = 500.}} These magnetic form factors (red lines) were derived from 1352 fits of the $^3$He world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. A $\chi^2_{max}$ = 500 cut was applied removing the nonphysical fits leaving 852 `good' fits. The blue line is the fit from \cite{Article:Amroun}.}
...
Figure ~\ref{fig:3h_fch_no_cut} shows the first set charge form factors from each of the 2600 fits from ~\ref{tab:3h_fits} without any $\chi^2_{max}$ cut plotted together. Plotted along with this analysis' fits in red is a blue line representing the average result from \cite{Article:Amroun} in the range that analysis considered its fits to be valid. It is important to note that no new data has been added for $^3$H so we do not expect our results to diverge much from past results like \cite{Article:Amroun}. This makes the $^3$H results more useful to check the consistency of our methodology with past results and as a point of comparison to the $^3$He results. It was initially hoped that new $^3$H data would be available for this analysis from the planned experiment E12-14-009 \cite{3h_proposal} which was the original basis for this thesis. Unfortunately, due to budget uncertainty and continuing resolutions this experiment was cancelled. Happily, Hall A was able to take some elastic $^3$H data in the end, and once this data has been analyzed it should be a simple matter to add it to this analysis.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=1.1\linewidth]{Fch_3H_n8_2600.png}
\end{center}
\caption[\bf{Charge Form Factors from 2600 $^3$H Fits}]{
{\bf{Charge Form Factors from 2600 $^3$H Fits.}} These charge form factors (red lines) were derived from 2600 fits of the $^3$H world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. No $\chi^2_{max}$ cut was applied resulting in many nonphysical fits. The blue line is the fit from \cite{Article:Amroun}.}
\label{fig:3h_fch_no_cut}
\end{figure}

Again, we notice that many of the fits are behaving in a nonphysical manner. We observe numerous dip only minima where the first sharp minima is expected. These are even more shallow than the $^3$He dips and often almost miss the first minima entirely. There are even two fits that seemingly failed to converge at all and stand out wildly from the others. The location of the first minima looks to be in good agreement with previous measurements even without the nonphysical fits being removed. The second minima, however, is barely pinned down at all. It seems as though it could be located anywhere from Q$^2$ = 26 - 60+ fm$^{-2}$ with a denser grouping around 30 fm$^{-2}$ or not exist at all according to the current data. 

Figure ~\ref{fig:3h_fch_cut} shows the resulting charge form factors of each of the 908 fits of the $^3$H world data that survive the $\chi^2_{max}$ = 603 cut plotted together. Immediate it is obvious that the cut has removed most of the nonphysical and bizarre looking results. The results remain in reasonable agreement with \cite{Article:Amroun} except the peak after the first minuma is higher and falls off faster. The cluster of second minima near 30 fm$^{-2}$ has almost entirely disappeared. There is still no determination of the location of a secondary diffractive minima with the current data indicating it could be located anywhere from  Q$^2$ = 30 - 60+ fm$^{-2}$ or not exist at all. There is simply not enough high precision data in the world data to make any determinations beyond approximately 25 fm$^{-2}$.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=1.1\linewidth]{Fch_3H_n8_908.png}
\end{center}
\caption[\bf{Charge Form Factors from 2600 $^3$H Fits $\chi^2_{max}$ = 603}]{
{\bf{Charge Form Factors from 2600 $^3$H Fits $\chi^2_{max}$ = 603.}} These charge form factors (red lines) were derived from 2600 fits of the $^3$H world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. A $\chi^2_{max}$ = 603 cut was applied removing the nonphysical fits leaving 908 `good' fits. The blue line is the fit from \cite{Article:Amroun}.}
\label{fig:3h_fch_cut}
\end{figure}

Figure ~\ref{fig:3h_fm_no_cut} shows the resulting magnetic form factors of each of the 2600 fits of the $^3$H world data without any $\chi^2_{max}$ cut plotted together. As with the charge form factor, with no cuts applied, there are many nonphysical fits. Most of these are dip only minima. The fits are decently consistent overall, and the location of the first minima is consistent with past results. Again, the magnetic form factor is less well determined than the charge form factor fits due to the relative lack of high Q$^2$ data. 

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=1.1\linewidth]{Fm_3H_n8_2600.png}
\end{center}
\caption[\bf{Magnetic Form Factors from 2600 $^3$H Fits}]{
{\bf{Magnetic Form Factors from 2600 $^3$H Fits.}} These magnetic form factors (red lines) were derived from 2600 fits of the $^3$H world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. No $\chi^2_{max}$ cut was applied resulting in many nonphysical fits. The blue line is the fit from \cite{Article:Amroun}.}
\label{fig:3h_fm_no_cut}
\end{figure}

Figure ~\ref{fig:3h_fm_cut} shows the resulting magnetic form factors of each of the 908 fits of the $^3$H world data that survive the $\chi^2_{max}$ = 603 cut plotted together. Once again, the cut has removed most of the nonphysical and bizarre looking results. After this cut our results still remain in agreement with \cite{Article:Amroun} and others. This is as we expect since no new $^3$H data was added to the world data. The fits indicate that a secondary diffractive minima could be anywhere from Q$^2$ = 36 - 60+ fm$^{-2}$ and offer little insight beyond this range. Collecting more high Q$^2$ data is again the best way to improve our knowledge of the $^3$H magnetic form factor.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=1.1\linewidth]{Fm_3H_n8_908.png}
\end{center}
\caption[\bf{Magnetic Form Factors from 2600 $^3$H Fits $\chi^2_{max}$ = 603}]{
{\bf{Magnetic Form Factors from 2600 $^3$H Fits $\chi^2_{max}$ = 603.}} These magnetic form factors (red lines) were derived from 2600 fits of the $^3$H world data using the pseudorandom R$_i$ values and the R$_i$ optimization procedure. A $\chi^2_{max}$ = 603 cut was applied removing the nonphysical fits leaving 908 `good' fits. The blue line is the fit from \cite{Article:Amroun}.}
\label{fig:3h_fm_cut}
\end{figure}
...
The shapes of the $^3$H charge densities of the `good' fits are fairly consistent, but where they reach the origin still has a much wider spread than $^3$He's charge density. Once again this is due to the dearth of data for $^3$H, especially at higher kinematics. Below a radius of about 0.5 fm the charge density turns over and comes to to a plateau. This turnover occurs at smaller radii for fits with higher charge density at low radii. Again, the charge density tapers off by a radii of 5 fm justifying the guess of R$_{max}$ in Section ~\ref{ssec:radii}.